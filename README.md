...
- **元件庫 41 種**：NFB、FUSE、MC 電磁接觸器、TR 限時電驛、MK 電力電驛、TH-RY 積熱電驛、CO 過電流電驛（51）、STOP/START 按鈕、COS 選擇開關、GL/RL 指示燈、BZ 蜂鳴器（會出聲）、三相馬達、六出線 Y-Δ 馬達、VM 電壓表、AM 電流表、SC 電容器組（功因改善）、TX 控制變壓器、**ATS 自動切換開關、GEN 柴油發電機（AMF 停電自起）**、小型 PLC（8 DI／8 DO）
...
- **45 個範例（四大分區）**：基礎工配（門鈴→自保持→寸動→MK 中繼→暖機延時⋯）、進階／電力配電（正逆轉、Y-Δ、雙 TR 閃爍、CO51、受電儀表盤、功因電容、ATS 停電切換、受電盤總和演練）、PLC（自保持、TON、CTU 計數、交替閃爍、正逆轉、Y-Δ、輸送帶雙向延時、斷續警報）
